...
async function calculatePayloadHash(payload: string, contentType: string) {
...
async function calculateMac(type: string, credentials: any, options: any) {
  ...
  return fromByteArray(new Uint8Array(hmac));
}

export async function header(method: string, uri: string, options: any) {
